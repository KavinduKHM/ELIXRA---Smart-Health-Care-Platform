...
const validateField = (name, value, selectedFile) => {
	...
	if (name === 'file') {
		if (!selectedFile) return 'Please select a document to upload.';
...
const UploadDocument = () => {
	...
	const validateForm = () => {
		...
		setTouched({ patientId: true, documentType: true, description: true, notes: true, file: true });
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			await axios.post(`${API_BASE_URL}/api/patients/${patientId}/documents`, payload, {
				headers: {
					...authHeaders,
			...
			navigate('/patient/documents', {
				...
				state: {
					flashMessage: {
						type: 'success',
						text: 'Document uploaded successfully to Cloudinary.'
					}
				}
			});
		} catch (uploadError) {
			...
			const msg = apiMsg ? `Upload failed: ${String(apiMsg)}` : 'Upload failed. Please try again.';
			setProgress(0);
			navigate('/patient/documents', {
				replace: true,
				state: {
		...
		<div className="ud-shell">
			<header className="ud-header">
				<div>
					...
					<p>Upload documents securely to Cloudinary via patient-service.</p>
				</div>
				<Link className="ud-link" to="/patient">Back to dashboard</Link>
			</header>
			...
			<form className="ud-form" onSubmit={handleSubmit} noValidate>
				...
				<label className="ud-field ud-field-full">
					<span>
						File<strong className="ud-required">*</strong>
					</span>
					<input
						type="file"
				...
				{uploading && (
					<div className="ud-progress-wrap ud-field-full">
						<div className="ud-progress-label">Uploading... {progress}%</div>
				...
				<footer className="ud-actions ud-field-full">
					<button className="ud-btn ud-btn-secondary" type="button" onClick={() => navigate('/patient')}>
						Cancel
					</button>
					<button className="ud-btn ud-btn-primary" type="submit" disabled={uploading}>
						<DocumentArrowUpIcon className="ud-btn-icon" />
